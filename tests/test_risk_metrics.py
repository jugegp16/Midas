...
class TestComputeRiskMetrics:
    ...
    def test_realized_vol_zero_when_window_contains_non_positive(self) -> None:
        # A non-positive bar inside the window must not be silently spliced
        # over: the resulting log-diff would fabricate a return between the
        # surviving points. Expect a clean zero fallback instead.
        values = [100.0] * 30 + [0.0] + [100.0] * 30
        curve = _equity_curve(values)
        metrics = compute_risk_metrics(curve, vol_target=None, per_strategy_pnl={})
        assert metrics.realized_vol_60d == 0.0
        assert metrics.rolling_sharpe_252d == 0.0
