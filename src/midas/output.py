...
def print_alert(
    ...
    *,
    ...
    """Render one BUY/SELL order alert as a bordered panel."""
...
def print_strategy_table(strategies: list[Strategy]) -> None:
    """Render the catalog of available strategies as a centered table."""
...
def _signed_dollar(value: float) -> str:
    """Dollar amount colored green/red based on sign."""
    style = "green" if value >= 0 else "red"
...
def _print_performance(result: BacktestResult) -> None:
    """Render the headline performance table plus a short-window caveat."""
...
def _print_after_tax(result: BacktestResult) -> None:
    """Render after-tax performance and per-year realized-tax tables."""
    if result.after_tax_final_value is None:
        return
    total_days = result.total_days
    ...
    for summary in result.tax_summary:
        tax_table.add_row(
            str(summary.year),
            f"${summary.st_realized:+,.2f}",
            f"${summary.lt_realized:+,.2f}",
            f"${summary.net_after_cross:+,.2f}",
            f"${summary.tax_owed:+,.2f}",
    ...
    total_st = sum(summary.st_realized for summary in result.tax_summary)
    total_lt = sum(summary.lt_realized for summary in result.tax_summary)
    total_tax = sum(summary.tax_owed for summary in result.tax_summary)
...
def _print_split(result: BacktestResult) -> None:
    """Render the train/test split table when the run had a split."""
    if not result.split_date:
        return
...
def _print_risk_metrics(result: BacktestResult) -> None:
    """Render the drawdown/Sharpe/Sortino and telemetry risk table."""
...
def _print_risk_engine_activity(result: BacktestResult) -> None:
    """Render CPPI/vol-target activity when either mechanic was live.

    Shown when CPPI fired on any bar OR when vol target is configured
    (regardless of whether it bound). The vol-target clause is
    ...
    if metrics is None or not (
        metrics.cppi_active_pct > 0 or metrics.cppi_min_scale < 1.0 or metrics.vol_target is not None
    ):
        return
    phase_table = make_metric_table("Risk Engine Activity")
    if metrics.cppi_active_pct > 0 or metrics.cppi_min_scale < 1.0:
        phase_table.add_row("CPPI Active (% of bars)", f"{metrics.cppi_active_pct:.1%}")
        phase_table.add_row("CPPI Avg Scale", f"{metrics.cppi_avg_scale:.2%}")
        phase_table.add_row("CPPI Min Scale", f"[red]{metrics.cppi_min_scale:.2%}[/red]")
    if metrics.vol_target is not None:
        phase_table.add_row("Vol Target Bound (% of bars)", f"{metrics.vol_target_bind_pct:.1%}")
        phase_table.add_row("Vol Target Avg Scale", f"{metrics.vol_target_avg_scale:.2%}")
    if metrics.vol_target_skip_count > 0:
        phase_table.add_row(
            "Vol Target Skipped (bars)",
            f"[yellow]{metrics.vol_target_skip_count}[/yellow]",
        )
...
def _print_vol_contribution(result: BacktestResult) -> None:
    ...
    Shares are time-averaged over the run, so a ticker exited mid-run
    keeps the risk it carried while held. Rows sum to ~100%.
    """
    if result.risk_metrics is None or not result.risk_metrics.per_ticker_vol_contribution:
        return
    contrib_table = make_metric_table("Per-Ticker Vol Contribution (time-averaged)")
    items = sorted(
...
def _print_pnl_attribution(result: BacktestResult) -> None:
    """Render per-strategy attributed P&L, highest first."""
    if result.risk_metrics is None or not result.risk_metrics.per_strategy_pnl:
        return
...
def _print_trade_quality(result: BacktestResult) -> None:
    """Render win-rate/profit-factor stats when any sell occurred."""
    if not any(trade.direction == Direction.SELL for trade in result.trades):
        return
...
def _print_strategy_aggregate(result: BacktestResult) -> None:
    """Render the per-strategy aggregate breakdown table."""
    ...
    for agg in aggregate_strategy_stats(result.strategy_stats):
        agg_table.add_row(
            ...
            _signed_dollar(agg.pnl) if agg.sells > 0 else "—",
        )
    ...
    agg_table.add_row(
        ...
        "—",
        "—",
        _signed_dollar(result.unrealized_pnl),
    )

    print_centered(agg_table)


def _print_strategy_detail(result: BacktestResult) -> None:
    """Render the per-(strategy, ticker) breakdown table and footnote."""
    ...
    for stat in result.strategy_stats:
        detail_table.add_row(
            ...
            _signed_dollar(stat.pnl) if stat.sells > 0 else "—",
        )
...
def print_backtest_summary(result: BacktestResult, *, show_charts: bool = False) -> None:
    """Print the full backtest summary — one centered table per section.

    Args:
        result: Completed backtest result to summarize.
        show_charts: When True, also render the terminal chart panel.
    """
    _print_performance(result)
    _print_after_tax(result)
    _print_split(result)
    _print_risk_metrics(result)
    _print_risk_engine_activity(result)
    _print_vol_contribution(result)
    _print_pnl_attribution(result)
    _print_trade_quality(result)
    if result.strategy_stats:
        _print_strategy_aggregate(result)
        _print_strategy_detail(result)
