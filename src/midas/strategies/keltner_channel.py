...
from midas.strategies.base import EntrySignal, rolling_mean, true_range_series


class KeltnerChannel(EntrySignal):
    """Entry signal: bullish when price breaks above the upper Keltner band."""
    ...
    def precompute(self, price_history: PriceHistory) -> np.ndarray | None:
        ...
        if num_bars < window or window < 2:
            return scores
        centerlines = rolling_mean(close, window)
        tr = true_range_series(price_history.high, price_history.low, close)
        atr_series = rolling_mean(tr, window)
    ...
    def score(
        ...
        **kwargs: object,
        ...
        centerline = float(close[-window:].mean())
        tr = true_range_series(price_history.high, price_history.low, close)
